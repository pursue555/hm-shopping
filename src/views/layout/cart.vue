    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="footer-fixed" >
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" :class="{ disabled: selCount === 0 }" @click="goPay" class="goPay">结算({{ selCount }})</div>
          <div v-else :class="{ disabled: selCount === 0 }"  @click="handleDel" class="delete">删除({{ selCount }})</div>
        </div>
      </div>
     </div>
...
export default {
  ...
  async created () {
  // 等待 Vuex 初始化完成（如从本地存储恢复 token）
    await this.$store.dispatch('user/checkLogin') // 确保 token 已同步到 Vuex
    if (this.isLogin) {
